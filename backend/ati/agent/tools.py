"""ATI AI Agent - 10 tools spec-exact.
...
from ati.models import Detection, Customer, CustomerAsset, RemediationAction, DetectionStatus, SeverityLevel
from ati.engine.exposure_scorer import get_customer_risk_summary
from ati.engine.playbooks import get_playbook, render_playbook_text
...
async def check_breach(email_or_domain: str) -> dict:
    ...
    headers = {"hibp-api-key": hibp_key, "User-Agent": "ATI/7.0"} if hibp_key else {}
...
async def search_paste_sites(search_term: str) -> dict:
    ...
    results = []
    async with httpx.AsyncClient(timeout=15.0, headers={"User-Agent": "ATI/7.0"}) as client:
...
async def generate_report(customer_name: str, period_days: int = 30) -> dict:
    ...
    from ati.services.report_generator import generate_pdf_report
...
async def update_alert_status(detection_id: int, new_status: str,
    ...
    async with async_session() as db:
        ...
        if new_status.upper() == "REMEDIATED":
            ...
            try:
                from ati.services.recheck_scheduler import schedule_recheck
...
async def search_related_findings(
    ...
    try:
        from ati.services.ai_rag_context import find_related_findings, get_actor_intelligence
        from ati.database import async_session
        async with async_session() as db:
...
TOOL_SCHEMAS = [
    {
        ...
        "function": {
            "name": "query_database",
            "description": "Search live threat detections in the ATI database. Returns real IOCs, severities, and statuses.",
            "parameters": {
                "type": "object",
                "properties": {
