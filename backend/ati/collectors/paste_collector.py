...
from ati.models import Detection, SeverityLevel, DetectionStatus
...
async def _fetch_pastes_json(url: str) -> list[dict]:
    ...
    try:
        async with httpx.AsyncClient(timeout=15.0, headers={"User-Agent": "ATI/8.0"}) as client:
...
async def _fetch_paste_content(paste_url: str) -> str:
    ...
    try:
        async with httpx.AsyncClient(timeout=10.0, follow_redirects=True,
                                      headers={"User-Agent": "ATI/8.0"}) as client:
