...
from ati.models import Detection, DarkWebMention, SeverityLevel, DetectionStatus, CustomerAsset, Customer
...
async def _fetch_rss(url: str, client: httpx.AsyncClient) -> list[dict]:
    try:
        resp = await client.get(url, timeout=20.0, headers={"User-Agent": "ATI/8.0"})
