"""ATI (Agentic Threat Intelligence) V16.4.7 - FastAPI backend."""
import os
import re
import logging
import socket
...
from ati.database import get_db
from ati.models import (Detection, SeverityLevel, DetectionStatus, Customer,
    CustomerAsset, ThreatActor, CustomerExposure, DarkWebMention, CollectorRun, Enrichment, Finding)
from ati.api.customers import router as customers_router
from ati.api.detections import router as detections_router
from ati.api.enrichments import enrich_router, remed_router
from ati.api.ai_routes import router as ai_router
from ati.api.stats_routes import router as stats_router
from ati.api.findings_routes import router as findings_router
from ati.api.ops_routes import router as ops_router
from ati.api.settings_routes import router as settings_router
...
async def lifespan(app: FastAPI):
    print(f"{'='*55}\n  ATI (Agentic Threat Intelligence) V16.4.7 -- Starting\n{'='*55}")

    # Disable AI during boot to prevent 2+ hour hang (149 findings × 60s per AI call)
    try:
        import ati.services.ai_pipeline_hooks as _ai_hooks
    ...
    try:
        from ati.engine.ioc_registry import seed_from_legacy
        from ati.database import async_session as _reg_session
    ...
    try:
        from ati.engine.ai_prompt_manager import seed_default_prompts
        from ati.database import async_session as _prompt_session
    ...
    async def auto_bootstrap():
        ...
        if proxy_ok:
            ...
            from ati.models import Detection
            from sqlalchemy import select, func as _bfunc
            for wait_round in range(6):
                await asyncio.sleep(10)
                from ati.database import async_session as _as
            ...
            from ati.database import async_session
            from ati.models import Detection, ThreatActor, DarkWebMention, CollectorRun
        ...
        try:
            from ati.engine.correlation_engine import correlate_new_detections
            from ati.database import async_session
            async with async_session() as db:
        ...
        try:
            from ati.engine.finding_manager import get_or_create_finding
            from ati.models import Detection
            from ati.database import async_session
            async with async_session() as db:
        ...
        try:
            from ati.engine.customer_intel_matcher import match_all_customers
            from ati.database import async_session
            async with async_session() as db:
        ...
        try:
            from ati.engine.attribution_engine import run_attribution_pass
            from ati.database import async_session
            async with async_session() as db:
        ...
        try:
            from ati.engine.campaign_detector import check_and_create_campaign
        ...
        try:
            from ati.models import Customer as _SeedCustomer
            from ati.database import async_session as _seed_session
        ...
        print(f"{'='*55}\n  ATI (Agentic Threat Intelligence) V16.4.7 -- READY (Real Intel)")
        ...
        try:
            from ati.services.ai_pipeline_hooks import _provider, _pipeline_ai_available
            prov = _provider()
            available = _pipeline_ai_available()
            print(f"  AI Engine:   {prov.upper()} {'✅ ACTIVE' if available else '❌ NOT AVAILABLE'}")
            if prov == "ollama":
                from ati.config import settings as _s
...
app = FastAPI(title="ATI (Agentic Threat Intelligence)", version="16.4.7", lifespan=lifespan)
...
from ati.api.enrichments import playbook_router, export_router
app.include_router(playbook_router)
app.include_router(export_router)

# V16.4.7: IOC Registry Admin API
from ati.api.admin_routes import router as admin_router
...
async def onboard_customer(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        try:
            from ati.tasks import retry_recon
    ...
    try:
        from ati.engine.customer_intel_matcher import match_customer_intel
    ...
    try:
        ...
        from ati.engine.customer_intel_matcher import _post_match_pipeline
    ...
    try:
        from ati.models import FindingRemediation, Campaign
    ...
    try:
        from ati.models import ExposureHistory
    ...
    async def _delayed_rematch():
        ...
        try:
            from ati.database import async_session
            from ati.engine.customer_intel_matcher import match_customer_intel, _post_match_pipeline
            from ati.services.exposure_scorer import calculate_customer_exposure
...
async def customer_collection_status(cid: int, db: AsyncSession = Depends(get_db)):
    ...
    from ati.models import CollectorRun
...
async def customer_breach_status(cid: int, db: AsyncSession = Depends(get_db)):
    ...
    from ati.models import Customer, Finding, Detection, DarkWebMention
...
async def ingest_edr(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    from ati.engine.edr_correlator import ingest_edr_telemetry
...
async def correlate_edr(customer_id: int, db: AsyncSession = Depends(get_db)):
    ...
    from ati.engine.edr_correlator import correlate_edr_hashes
...
async def enrich_detection_endpoint(detection_id: int):
    from ati.services.enrichment_pipeline import enrich_detection
...
async def enrich_batch(limit: int = 20, db: AsyncSession = Depends(get_db)):
    ...
    from ati.services.enrichment_pipeline import enrich_detection
    from ati.models import Enrichment as EnrichModel
    r = await db.execute(
...
async def update_remediation(action_id: int, status: str,
    ...
    from ati.models import RemediationAction
    from ati.services.recheck import schedule_recheck
