...
async def dispatch_detection_alert(detection, customer=None) -> dict:
    ...
    if alert_email:
        results["email"] = send_email(
            subject=f"[ATI] {sev}: {detection.ioc_type} - {customer_name}",
...
async def dispatch_escalation_alert(detection, level: int = 2, customer=None) -> dict:
    ...
    if level == 2:
        ...
        msg = (
            ...
            f"IR retainer engagement required. ATI: http://localhost:7777"
        )
    ...
    if ciso_email:
        results["ciso_email"] = send_email(
            subject=f"[ATI ESCALATION L{level}] {customer_name}",
