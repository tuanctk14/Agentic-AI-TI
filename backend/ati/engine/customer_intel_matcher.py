...
async def match_customer_intel(customer_id: int, db: AsyncSession) -> dict:
   ...
   - no industry set -> WARNING, D3 scoring will be zero
    ...
    if all_domains:
        from ati.utils import extract_domains_from_token
...
async def _post_match_pipeline(finding_id: int, db: AsyncSession):
    ...
    try:
        from ati.engine.severity_scorer import score as score_ioc, get_mitre_context
    ...
    try:
        from ati.engine.ioc_registry import get_registry, calculate_dynamic_severity
        ...
        if reg_entry.get("auto_score_enabled", True):
            ...
            _enrich_flat = {}
            try:
                from ati.models import Enrichment, Detection
    ...
    try:
        from ati.services.ai_pipeline_hooks import hook_ai_triage
...
async def match_all_customers(db: AsyncSession) -> dict:
    ...
    for customer in customers:
        try:
            from ati.engine.threat_pressure import calculate_probable_exposures
