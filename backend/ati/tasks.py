        ...
        async with async_session() as db:
            ...
            if total > 0:
                try:
                    ...
                    from ati.models import Finding, FindingRemediation
...
def check_sla_and_alert_task():
    ...
    from ati.models import Finding, Customer, DetectionStatus, SeverityLevel
    from ati.engine.alert_dispatcher import dispatch_finding_alert, send_slack, send_email
        ...
        async with async_session() as db:
            ...
            for finding in breached:
                try:
                    ...
                    if alert_email:
                        send_email(
                            subject=f"[ATI SLA BREACH] {sev_val}: {customer.name}",
...
def threat_pressure_task():
    ...
    """
    from ati.database import async_session
    from ati.engine.threat_pressure import calculate_threat_pressure
...
def exposure_recalc_task():
    ...
    async def _recalc():
        async with async_session() as db:
            ...
            try:
                from ati.engine.exposure_narrative import generate_all_narratives
...
def snapshot_exposure_history():
    ...
    async def _snapshot():
        from ati.models import Customer, Detection, ExposureHistory
...
def retry_recon(self, customer_id, domain):
    ...
    async def _retry():
        from ati.database import async_session
        from ati.models import Customer
        from sqlalchemy import select
...
def data_cleanup_task():
    ...
    async def _cleanup():
        ...
        from ati.models import Detection, CollectorRun, Enrichment, DetectionStatus
...
def mitre_sync_task(self):
    ...
    async def _run():
        from ati.database import async_session
        from ati.engine.ai_prompt_manager import sync_mitre_attack
        async with async_session() as db:
