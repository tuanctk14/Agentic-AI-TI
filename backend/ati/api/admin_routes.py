...
from ati.engine.ioc_registry import (
...
async def get_weights(db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        from ati.engine.ioc_registry import _DEFAULT_WEIGHTS
...
async def list_prompts(db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        from ati.engine.ai_prompt_manager import DEFAULT_PROMPTS
...
async def get_prompt_detail(hook_name: str, db: AsyncSession = Depends(get_db)):
    ...
    from ati.engine.ai_prompt_manager import get_prompt
...
async def update_prompt(hook_name: str, request: Request, db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        await db.commit()
        from ati.engine.ai_prompt_manager import _prompt_cache
...
async def test_prompt(hook_name: str, request: Request, db: AsyncSession = Depends(get_db)):
    ...
    try:
        from ati.agent.agent_core import _resolve_provider
