"""ATI Settings Routes -  Extracted from main.py"""
import os
import json
import logging
...
from ati.auth import require_role
...
async def list_actor_iocs(actor_id: int = None, ioc_type: str = None,
    ...
    from ati.models import ActorIoc
    q = select(ActorIoc)
    if actor_id:
...
async def create_actor_ioc(
    ...
    from ati.models import ActorIoc, ThreatActor
...
async def agent_query(req: AgentQuery):
    from ati.agent.agent_core import run_agent
...
async def get_ai_settings():
    ...
    from ati.agent.tools import TOOL_REGISTRY
...
async def agent_provider_health():
    ...
    from ati.agent.agent_core import check_provider_health
    from ati.config import settings
    from ati.services.ai_pipeline_hooks import _provider, _get_active_provider_from_redis
...
async def set_active_provider(request: Request):
    ...
    """
    from ati.services.ai_pipeline_hooks import _set_active_provider_in_redis, _provider
...
async def customer_exposure_scores(customer_id: int):
    from ati.services.exposure_scorer import get_customer_top_threats
...
async def exposure_leaderboard(sector: str = None, db: AsyncSession = Depends(get_db)):
    ...
    from ati.models import CustomerExposure, Customer as CustomerModel, ExposureHistory
    ...
    for row in r:
        ...
        if score == 0:
            from ati.models import Finding as FindingModel
...
async def exposure_ai_interpretation(customer_id: int, db: AsyncSession = Depends(get_db)):
    ...
    from ati.models import ExposureHistory, Customer as CustModel
    from ati.services.ai_pipeline_hooks import hook_ai_exposure_interpretation
...
async def generate_report(customer_id: int, period_days: int = 30):
    from ati.services.pdf_report import generate_pdf_report
...
async def export_stix(detection_id: int, db: AsyncSession = Depends(get_db)):
    from ati.engine.stix_exporter import export_detection_to_stix
...
async def cve_attribution(cve_id: str, db: AsyncSession = Depends(get_db)):
    from ati.engine.attribution_engine import CVE_ACTOR_MAP
...
async def enrich_detection_attribution(detection_id: int, db: AsyncSession = Depends(get_db)):
    from ati.engine.attribution_engine import attribute_detection_by_id
...
async def get_customer_risk(customer_id: int, db: AsyncSession = Depends(get_db)):
    from ati.services.exposure_scorer import get_customer_risk_summary
...
async def customer_exposure_breakdown(cid: int, db: AsyncSession = Depends(get_db)):
    ...
    from ati.models import Customer, CustomerExposure, ThreatActor, ExposureHistory, Finding
...
async def export_stix_bulk():
    ...
    from ati.engine.stix_exporter import export_all_to_stix
...
async def export_siem():
    from ati.engine.syslog_exporter import send_recent_to_siem
...
async def discover_assets(cid: int, request: Request):
    ...
    from ati.services.asset_discovery import (
    ...
    from ati.models import Customer, CustomerAsset
...
async def get_agent_schema():
    ...
    from ati.services.asset_discovery import AGENT_SCHEMA
    return AGENT_SCHEMA
...
async def list_discovery_providers():
    ...
    from ati.services.discovery_providers import get_configured_providers
...
async def discover_external(cid: int, provider: str = ""):
    ...
    """
    from ati.services.asset_discovery import AssetRecord, ingest_assets
    from ati.services.osint_discovery import run_osint_discovery
    from ati.models import Customer, CustomerAsset, AssetType
    from ati.database import async_session
    from sqlalchemy import select
...
async def list_sector_advisories(
    ...
    from ati.models import SectorAdvisory
...
async def trigger_sector_detection(db: AsyncSession = Depends(get_db)):
    ...
    from ati.engine.sector_detection import detect_sector_campaigns
...
async def trigger_darkweb_triage(db: AsyncSession = Depends(get_db)):
    ...
    from ati.engine.darkweb_triage import triage_untriaged_mentions
...
async def get_exposure_narrative(customer_id: int, db: AsyncSession = Depends(get_db)):
    ...
    from ati.models import CustomerExposure
    r = await db.execute(
...
async def set_ai_keys(request: Request):
    ...
    if updated and updated[0] in active and updated[0] in ai_providers:
        from ati.services.ai_pipeline_hooks import _set_active_provider_in_redis
...
async def agent_status(db: AsyncSession = Depends(get_db)):
    ...
    from ati.models import FPPattern, SectorAdvisory, DarkWebMention, CustomerExposure
    from sqlalchemy import func as _fn
    from ati.services.ai_pipeline_hooks import _pipeline_ai_available
