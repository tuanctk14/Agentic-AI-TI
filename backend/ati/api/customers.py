...
from ati.api.schemas import CustomerCreate, CustomerUpdate, CustomerOut, AssetCreate, AssetOut
...
async def list_customers(active_only: bool = True, db: AsyncSession = Depends(get_db)):
    ...
    from ati.models import Detection, CustomerAsset as CA
    q = select(Customer)
    if active_only:
    ...
    for c in customers:
        ...
        score_source = "none"
        from ati.models import CustomerExposure, ExposureHistory
...
async def advance_onboarding(cid: int, state: str = "", db: AsyncSession = Depends(get_db)):
    ...
    from ati.models import CustomerAsset, Detection
