"""
ATI AI Routes -  Extracted from main.py
...
async def ai_triage_endpoint(limit: int = 5, db: AsyncSession = Depends(get_db)):
    ...
    from ati.models import Finding, Customer, SeverityLevel
...
async def pipeline_fixup_endpoint(db: AsyncSession = Depends(get_db)):
    ...
    from ati.models import Finding, Detection, Customer, FindingRemediation
...
async def ai_query(req: AIQuery, db: AsyncSession = Depends(get_db)):
    ...
    actors_summary = ""
    try:
        from ati.models import ThreatActor
        ar = await db.execute(
...
"""
                ...
                try:
                    from ati.models import CustomerAsset as CA2
    ...
    context = f"""You are ATI AI, a cybersecurity threat intelligence analyst for an MSSP platform.
...
async def agentic_chat_query(req: AIQuery, db: AsyncSession = Depends(get_db)):
    ...
    from ati.agent.chat_tools import CHAT_TOOLS, CHAT_TOOL_SCHEMAS, set_db
    ...
    # Build system prompt
    system = """You are ATI AI, a cybersecurity threat intelligence analyst with access to the platform database.
...
Keep answers concise (under 250 words). Use the exact numbers and names from tool results."""
    ...
    if prov == "anthropic":
        from ati.agent.agent_core import _call_anthropic as _call_llm_fn
    elif prov == "openai":
        from ati.agent.agent_core import _call_openai as _call_llm_fn
    elif prov == "google":
        from ati.agent.agent_core import _call_google as _call_llm_fn
    else:
        from ati.agent.agent_core import _call_ollama as _call_llm_fn
...
async def reliable_chat_endpoint(req: AIQuery, db: AsyncSession = Depends(get_db)):
    ...
    from ati.agent.chat_agent_reliable import reliable_chat
...
async def investigate_endpoint(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    from ati.agent.investigate_agent import investigate_ioc
...
async def ai_match_confidence_endpoint(limit: int = 5, db: AsyncSession = Depends(get_db)):
    ...
    Default: 5 at a time.
    """
    from ati.services.ai_pipeline_hooks import hook_ai_match_confidence, _pipeline_ai_available, _provider
