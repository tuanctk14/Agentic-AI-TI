"""ATI Findings Routes -  Extracted from main.py"""
import os
import json
import logging
...
async def get_finding(finding_id: int, db: AsyncSession = Depends(get_db)):
    ...
    from ati.models import Finding, FindingSource, FindingRemediation, ThreatActor, Campaign
    ...
    if f.matched_asset and f.customer_id:
        from ati.models import CustomerAsset
...
async def update_finding_status(finding_id: int, request: Request, status: str = None, db: AsyncSession = Depends(get_db)):
    ...
    if new_severity and new_severity.upper() != str(f.severity.value if hasattr(f.severity, 'value') else f.severity).upper():
        ...
        try:
            from ati.models import SeverityLevel
    ...
    if status and status.upper() == "FALSE_POSITIVE" and f.customer_id:
        try:
            from ati.engine.fp_memory import record_fp_pattern
...
async def update_remediation_status(
    ...
    from ati.models import FindingRemediation
    from datetime import datetime, timezone
    r = await db.execute(
...
async def list_campaigns(
    ...
    from ati.models import Campaign, Customer
    q = select(Campaign)
...
async def get_campaign(campaign_id: int, db: AsyncSession = Depends(get_db)):
    ...
    from ati.models import Campaign, Finding, Customer, FindingSource, FindingRemediation
...
async def list_all_remediations(status: Optional[str] = None, db: AsyncSession = Depends(get_db)):
    ...
    from ati.models import FindingRemediation, Finding, Customer
