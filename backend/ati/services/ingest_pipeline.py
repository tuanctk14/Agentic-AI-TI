...
async def _async_pipeline(detection_id: int) -> dict:
    ...
    from ati.models import Detection, Customer, SeverityLevel
    ...
    try:
        ...
        try:
            from ati.services.ai_pipeline_orchestrator import ai_orchestrate_detection
        ...
        async with async_session() as db:
            ...
            if not det.customer_id:
                from ati.engine.correlation_engine import route_detection
        ...
        if should_enrich and not _orchestration_handled_severity:
            ...
            try:
                async with async_session() as db:
                    from ati.models import Finding as _F5, Customer as _C5, SeverityLevel as _SL5
                    from ati.services.ai_pipeline_hooks import (
                    ...
                    if _f5:
                        ...
                        from ati.engine.enrichment_feedback import process_enrichment_feedback
                        ...
                        try:
                            from ati.engine.fp_memory import check_fp_history
                            if _f5.customer_id:
                                ...
                                if _fp_check and _fp_check.get("auto_close"):
                                    ...
                                    from ati.models import DetectionStatus as _DS_FP
                        ...
                        _raw_text_5a = ""
                        try:
                            from ati.models import Detection as _Det5a
                        ...
                        try:
                            ...
                            if _ai_fp and _ai_fp.get("is_fp") and _ai_fp.get("confidence", 0) > 0.75:
                                ...
                                from ati.config import settings as _s5b
                                if getattr(_s5b, "AI_AUTONOMOUS", False):
                                    # AUTONOMOUS: auto-close the finding
                                    from ati.models import DetectionStatus as _DS5
        ...
        try:
            async with async_session() as db:
                from ati.models import Finding, ThreatActor, Customer as _C6
                from ati.engine.attribution_engine import (
                    attribute_finding, get_candidate_actors, update_customer_exposure
                )
                from ati.services.ai_pipeline_hooks import hook_attribution_assist
                rf = await db.execute(select(Finding).where(Finding.id == finding_id))
        ...
        try:
            async with async_session() as db:
                from ati.models import Finding, Customer as _C65, CveProductMap as _CPM65
                from ati.services.ai_pipeline_hooks import hook_rescore_severity
                from ati.config import settings as _s65
                ...
                if _f65 and not _orchestration_handled_severity:
                    ...
                    if _triage_confident and not _actor_added_context:
                        ...
                        from ati.models import Enrichment as _E65, Detection as _D65
                        ...
                        if _rescore and _rescore.get("severity"):
                            from ati.models import SeverityLevel as _SL65
                            try:
            ...
            async with async_session() as db:
                ...
                if finding and finding.actor_id:
                    ...
                    if campaign:
                        ...
                        try:
                            from ati.services.ai_pipeline_hooks import hook_campaign_narrative
                            # Get customer name
                            cust_name = None
                            if finding.customer_id:
            ...
            async with async_session() as db:
                ...
                if finding and finding.customer_id and finding.actor_id:
                    ...
                    if sev_val in ("CRITICAL", "HIGH"):
                        from ati.services.exposure_scorer import score_customer_actor
                        from ati.models import CustomerExposure, Customer as Cust, ThreatActor
                        ...
                        if customer and actor:
                            ...
                            if exp:
                                ...
                                from ati.models import CustomerExposure as CE
            ...
            try:
                async with async_session() as db:
                    ...
                    if finding and sev_val in ("CRITICAL", "HIGH", "MEDIUM"):
                        from ati.engine.action_generator import generate_action
        ...
        try:
            async with async_session() as db:
                from ati.models import Finding, Customer
                from ati.engine.alert_dispatcher import dispatch_detection_alert
                rf = await db.execute(select(Finding).where(Finding.id == finding_id))
                finding = rf.scalar_one_or_none()
                sev_val = _sev(finding.severity) if finding and finding.severity else "LOW"
                if finding and sev_val in ("CRITICAL", "HIGH"):
                    ...
                    from ati.engine.alert_dispatcher import dispatch_finding_alert
...
async def _async_batch_pipeline() -> dict:
    ...
    from ati.database import async_session
    from ati.models import Detection, DetectionStatus
...
async def _async_recheck() -> dict:
    ...
    from ati.database import async_session
    from ati.models import Finding, FindingRemediation, DetectionStatus, SeverityLevel
    from sqlalchemy import select, and_
...
async def _reopen_recurrent_findings() -> dict:
    ...
    from ati.database import async_session
    from ati.models import Finding, Detection, DetectionStatus
    from sqlalchemy import select, and_
    from datetime import datetime, timezone
