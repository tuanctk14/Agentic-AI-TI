...
from ati.agent.agent_core import SYSTEM_PROMPT
...
def _provider() -> str:
    ...
    try:
        from ati.services.ai_pipeline_hooks import _get_active_provider_from_redis
...
async def _tool_get_context(detection_id: int, finding_id: int | None = None) -> dict:
    ...
    from ati.models import Detection, Finding, Customer, ThreatActor
...
async def _tool_get_candidates(detection_id: int, finding_id: int | None = None) -> dict:
    ...
    from ati.database import async_session
    from ati.models import Detection, Finding
    from ati.engine.attribution_engine import get_candidate_actors
    from sqlalchemy import select
...
async def _tool_set_severity(
    ...
    from ati.database import async_session
    from ati.models import Finding, SeverityLevel
    from sqlalchemy import select
...
async def _tool_set_actor(
    ...
    from ati.database import async_session
    from ati.models import Finding, ThreatActor
    from sqlalchemy import select
...
async def _tool_flag_fp(finding_id: int, reason: str, confidence: float) -> dict:
    ...
    from ati.database import async_session
    from ati.models import Finding, DetectionStatus
    from ati.config import settings as _sfp
    from sqlalchemy import select
...
ORCHESTRATOR_SYSTEM = """You are ATI AI Pipeline Orchestrator - an autonomous cybersecurity analyst running inside a live threat detection platform.
...
async def ai_orchestrate_detection(detection_id: int) -> dict:
    ...
    try:
        if provider == "anthropic":
            from ati.agent.agent_core import _call_anthropic
